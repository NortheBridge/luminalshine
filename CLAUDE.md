...
- `SUNSHINE_ENABLE_WEBRTC` — **ON by default on Windows** since 26.09.1 (OFF elsewhere). `cmake/dependencies/webrtc.cmake` downloads the prebuilt NortheBridge/libwebrtc wrapper (pinned by `LIBWEBRTC_TAG` + `LIBWEBRTC_SHA256`) into `%LOCALAPPDATA%\LuminalShine\deps\libwebrtc\prebuilt\<tag>` when no local wrapper is found; `-DSUNSHINE_ENABLE_WEBRTC=OFF` opts out, `-DWEBRTC_ROOT=<dir>` uses your own. Building the wrapper from source (`scripts/build_mingw_webrtc.ps1`, cache `%LOCALAPPDATA%\LuminalShine\deps\libwebrtc\{src,out}`, **shared across worktrees**) is only needed to change the wrapper itself. Bump the tag together with the `third-party/libwebrtc` submodule.
