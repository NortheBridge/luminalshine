...
- **The bootstrapper ↔ MSI property contract stays identical.** The bootstrapper
  ...
  the same semantics:
  `INSTALL_ROOT`, `INSTALL_SUDOVDA`, `REMOVEVIRTUALDISPLAYDRIVER`,
...
| **PR 3** — Cut over to WiX 7 (single step) | ✅ merged | [#29](https://github.com/NortheBridge/luminalshine/pull/29) (`7d015447`) | **Collapsed from old PR 3 + PR 4.** WiX 7 toolchain pinned in `.config/dotnet-tools.json`. `luminalshine.wxs` + `custom_actions.wxs` + the generator-emitted `files.wxs` / `features.wxs` translated to v4 namespace (`http://wixtoolset.org/schemas/v4/wxs`): `<Product>`→`<Package>` merged, `<Directory ProgramFiles64Folder>`→`<StandardDirectory>`, `Win64="yes"`→`Bitness="always64"`, top-level `<Condition>`→`<Launch>`, `<Custom>` condition-text→`Condition=` attribute, `Feature Absent="disallow"`→`AllowAbsent="no"`, feature-level `<Condition Level=>`→`<Level Value= Condition=>`, `BinaryKey="WixCA"`→`BinaryRef="Wix4UtilCA_X64"` (util extension's renamed CA binary; entry-point names unchanged), `<UIRef>`→`<ui:WixUI>`. `windows_wix.cmake` rewritten to drive `dotnet wix build` directly; the WiX 3 candle/light path is gone. CI workflow installs the v7 toolchain via `dotnet tool restore` + `dotnet wix eula accept wix7` + `dotnet wix extension add` (UI / Util / Firewall). Golden re-baselined post-cutover (toolchain fingerprints differ; upgrade invariants byte-identical). |
| **PR 4** — Permanent CI guardrails | 🟡 iterating | — (`wix7-migration-pr4`) | New `scripts/assert_msi_invariants.py` enforces pinned upgrade invariants (`UpgradeCode`, `ServiceInstall` names, hand-authored Component GUIDs, bootstrapper-contract Property declarations) as a hard CI gate independent of the table-diff oracle — so a future deliberate re-baseline can't silently drop one of these values. `Condition` MSI table added to `scripts/dump_msi_tables.ps1`'s `$TablesToDump` so feature-level Conditions (the `INSTALL_SUDOVDA` gating rows) are diffed; baseline regenerated to include them. Final docs pass ([CLAUDE.md](../CLAUDE.md) build section, this file). |
